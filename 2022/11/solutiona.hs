...
catchItems :: Monkey -> M.Map Int Monkey -> M.Map Int Monkey
catchItems Monkey {..} monkeys = foldl' throw monkeys items
  where adjustWorry = (`div` 3) . operation
...
throwItems :: Int -> M.Map Int Monkey -> M.Map Int Monkey
throwItems monkey monkeys = M.insert monkey n (catchItems m monkeys)
  where m = monkeys M.! monkey
        n = m { items = [], inspections = inspections m + length (items m) }

playRound :: M.Map Int Monkey -> M.Map Int Monkey
playRound monkeys = foldr throwItems monkeys [size - 1, size - 2 .. 0]
  where size = M.size monkeys

simulate :: Int -> M.Map Int Monkey -> M.Map Int Monkey
simulate 0 = id
simulate n = simulate (n - 1) . playRound
...
main :: IO ()
main =
  fmap (monkeyBusiness . simulate 20) . parseMonkeys <$> getContents >>= \case

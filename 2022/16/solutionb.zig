...
const Neighborhood = struct {
    ...
    pub fn next(self: *Self, agent: usize) !?State {
        const n = self.graph.adjacency.count();
        if (self.index >= n) return null;
        defer self.index += 1;

        const state = self.state;
        const current_idx = self.graph.mapping.get(state.current[agent]).?.idx;
        ...
        while (self.index < n) : (self.index += 1) {
            ...
            const remaining_time = state.remaining_time[agent] - dist - 1;
            ...
            if (neighbor_rate > 0 and remaining_time >= 0 and !state.opened.contains(neighbor)) {
                ...
                var new_current = [_]u16{ undefined, undefined };
                new_current[agent] = neighbor;
                new_current[1 - agent] = state.current[1 - agent];

                var new_remaining_time = [_]i32{ undefined, undefined };
                new_remaining_time[agent] = remaining_time;
                new_remaining_time[1 - agent] = state.remaining_time[1 - agent];
...
fn search(graph: Graph, start_label: u16, allocator: mem.Allocator) !i32 {
    ...
    const initial = State{
        .current = [_]u16{ start_label, start_label },
        .opened = std.AutoHashMap(u16, void).init(allocator),
        .remaining_time = [_]i32{ time_limit, time_limit },
        .utility = 0,
    };
    ...
    while (queue.len > 0) {
        ...
        while (try neighborhood.next(0)) |neighbor| try queue.add(neighbor);
        neighborhood.reset();
        while (try neighborhood.next(1)) |neighbor| try queue.add(neighbor);
    }

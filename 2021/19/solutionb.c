    ...
    for (i = 0; i < s1->nbeacons; ++i) {
      for (j = 0; j < s2->nbeacons; ++j) {
        ...
        if (scanners_intersection(s1, t) >= 12) {
          s2->position[0] = delta[0];
          s2->position[1] = delta[1];
          s2->position[2] = delta[2];
          return 1;
        }
      }
    }
...
{
  char buf[32] = {'\0'};
  size_t i = 0, j, nscanners = 0, nprocessed = 1, max_beacons = 0;
  int processed[MAX_SCANNERS] = {0}, dist, manhattan = 0;
  ...
  for (i = 0; i < nscanners; ++i) {
    for (j = 0; j < nscanners; ++j) {
      dist = ABS(scanners[i]->position[0] - scanners[j]->position[0])
           + ABS(scanners[i]->position[1] - scanners[j]->position[1])
           + ABS(scanners[i]->position[2] - scanners[j]->position[2]);
      if (dist > manhattan)
        manhattan = dist;
    }
  }

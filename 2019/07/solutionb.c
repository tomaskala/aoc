#include <errno.h>
#include <limits.h>
#include <stdio.h>
#include <stdlib.h>

#define MAX(x, y) ((x) > (y) ? (x) : (y))

int signal[] = {5, 6, 7, 8, 9};
int phase_set[] = {0, 0, 0, 0, 0};
int ips[] = {0, 0, 0, 0, 0};
int amp;
int prev_output;

enum instruction {
  ADD = 1,
  MULTIPLY = 2,
  INPUT = 3,
  OUTPUT = 4,
  JUMP_IF_TRUE = 5,
  JUMP_IF_FALSE = 6,
  LESS_THAN = 7,
  EQUALS = 8,
  HALT = 99,
};

enum parameter_mode {
  POSITION = 0,
  IMMEDIATE = 1,
};

enum exit_code {
  EXIT_OUTPUT,
  EXIT_HALT,
  EXIT_ERROR,
};

int arity[] = {
  [ADD] = 3,
  [MULTIPLY] = 3,
  [INPUT] = 1,
  [OUTPUT] = 1,
  [JUMP_IF_TRUE] = 2,
  [JUMP_IF_FALSE] = 2,
  [LESS_THAN] = 3,
  [EQUALS] = 3,
  [HALT] = 0,
};

int intcode_read(char *buf, int *out)
{
  int i;
  char *endptr = buf;
  for (i = 0; *endptr != '\0' && *endptr != '\n'; ++i) {
    out[i] = (int) strtol(buf, &endptr, 10);
    if (errno)
      return 0;
    buf = endptr + 1;
  }
  return i;
}

int decode(int *code, int ip, enum parameter_mode mode)
{
  switch (mode) {
  case POSITION:
    return code[code[ip]];
  case IMMEDIATE:
    return code[ip];
  default:
    fprintf(stderr, "Unexpected parameter mode '%d'\n", mode);
    exit(EXIT_FAILURE);
  }
}

int input()
{
  if (phase_set[amp])
    return prev_output;
  else {
    phase_set[amp] = 1;
    return signal[amp];
  }
}

void output(int val)
{
  prev_output = val;
}

enum exit_code intcode_interpret(int *code, int length)
{
  #define BINARY(op) \
    do { \
      int a = decode(code, ip + 1, mode % 10); \
      int b = decode(code, ip + 2, (mode / 10) % 10); \
      CHECK_WRITE_MODE(mode / 100); \
      STORE(ip + 3, a op b); \
    } while (0)
  #define STORE(i, val) (code[code[i]] = val)
  #define CHECK_WRITE_MODE(mode) \
    do { \
      if (mode != 0) { \
        fprintf(stderr, \
            "The write parameter for opcode %d must be in mode 0, got '%d'\n", \
            opcode, mode); \
        exit(EXIT_FAILURE); \
      } \
    } while (0)
  #define SAVE_AND_RETURN(ecode) \
    do { \
      ips[amp] = ip + arity[opcode] + 1; \
      return ecode; \
    } while (0)
  int ip = ips[amp];
  while (ip < length) {
    int mode = code[ip] / 100;
    int opcode = code[ip] % 100;
    switch (opcode) {
    case ADD: {
      BINARY(+);
      break;
    }
    case MULTIPLY: {
      BINARY(*);
      break;
    }
    case INPUT:
      CHECK_WRITE_MODE(mode);
      int val = input();
      STORE(ip + 1, val);
      break;
    case OUTPUT: {
      int val = decode(code, ip + 1, mode);
      output(val);
      SAVE_AND_RETURN(EXIT_OUTPUT);
    }
    case JUMP_IF_TRUE: {
      int cond = decode(code, ip + 1, mode % 10);
      if (cond != 0) {
        ip = decode(code, ip + 2, (mode / 10) % 10);
        continue;
      }
      break;
    }
    case JUMP_IF_FALSE: {
      int cond = decode(code, ip + 1, mode % 10);
      if (cond == 0) {
        ip = decode(code, ip + 2, (mode / 10) % 10);
        continue;
      }
      break;
    }
    case LESS_THAN:
      BINARY(<);
      break;
    case EQUALS:
      BINARY(==);
      break;
    case HALT:
      SAVE_AND_RETURN(EXIT_HALT);
    default:
      fprintf(stderr, "Unexpected instruction '%d' at position '%d'\n",
          opcode, ip);
      exit(EXIT_FAILURE);
    }
    ip += arity[opcode] + 1;
  }
  #undef SAVE_AND_RETURN
  #undef CHECK_WRITE_MODE
  #undef STORE
  #undef BINARY
  ips[amp] = ip;
  return EXIT_ERROR;
}

int next_permutation(int *arr, size_t len)
{
  int i = len - 1;
  for (; i > 0 && arr[i - 1] >= arr[i]; --i);
  if (i <= 0)
    return 0;
  int j = len - 1;
  for (; arr[j] <= arr[i - 1]; --j);
  int tmp = arr[i - 1];
  arr[i - 1] = arr[j];
  arr[j] = tmp;
  j = len - 1;
  while (i < j) {
    tmp = arr[i];
    arr[i] = arr[j];
    arr[j] = tmp;
    i++;
    j--;
  }
  return 1;
}

int main(void)
{
  char buf[4096] = {'\0'};
  int code[1024] = {0}, work[5][1024] = {0};
  int length, result = INT_MIN;
  if (!fgets(buf, sizeof(buf), stdin)) {
    fprintf(stderr, "Error reading input line\n");
    exit(EXIT_FAILURE);
  }
  if ((length = intcode_read(buf, code)) == 0) {
    fprintf(stderr, "Error parsing input\n");
    exit(EXIT_FAILURE);
  }
  do {
    prev_output = 0;
    for (int a = 0; a < 5; ++a) {
      for (int i = 0; i < length; ++i)
        work[a][i] = code[i];
      phase_set[a] = 0;
      ips[a] = 0;
    }
    for (amp = 0;; amp = (amp + 1) % 5) {
      enum exit_code ecode = intcode_interpret(work[amp], length);
      if (ecode == EXIT_HALT)
        break;
      else if (ecode == EXIT_FAILURE) {
        fprintf(stderr, "Error during execution\n");
        exit(EXIT_FAILURE);
      }
    }
    result = MAX(result, prev_output);
  } while (next_permutation(signal, 5));
  printf("%d\n", result);
  return EXIT_SUCCESS;
}

...
function countEnclosedAreas(lines: Array<string>, start: Coords): number {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      ...
      while (iCurr < lines.length && jCurr < line.length) {
        const char = lines[iCurr][jCurr];

        if (dists.has(`${iCurr},${jCurr}`) && char != "7" && char != "L") {
          crossings++;
        }
...
rl.on("close", () => {
  ...
  console.log(countEnclosedAreas(lines, startCoords));
});
